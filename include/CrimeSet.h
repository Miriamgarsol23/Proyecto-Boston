...
public:
     ...
     * @return A const reference to the Crime object at position @p index
     */
    Crime operator[](int index) const;   
...
private:
    ...
    void allocate(int capacity);      // Asigna nueva memoria para _crimes
    void reallocate(int capacity);    // Aumenta y copia a un nuevo array
    void deallocate();                   //Libera memoria dinámica
    void copy(const CrimeSet &other);    // Copia de otro CrimeSet
 ...
 * of each line
 */
std::string FormatAsComment(const std::string &comment, char commentCharacter='#');
