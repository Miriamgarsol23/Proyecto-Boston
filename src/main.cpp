...
int main(int argc, char* argv[]) {
    string ofilename = "tests/output/output.ppm";
    string comment_extra = "";
    string parametros = "";
    Coordinates topRightCoor = CrimeCounter::DEFAULT_COORD_TOPRIGHT;
    Coordinates bottonLeftCoor = CrimeCounter::DEFAULT_COORD_BOTTOMLEFT;
    int rows = 19, cols = 23;
    int tonesD = 2, tonesB = 2, tonesG = 2, tonesY = 2;

    CrimeSet crimeSet, currentCrimeSet;
    ...
    ifstream fcheck;
    fcheck.open(argv[firstInputFile]);
    if (fcheck) {
        fcheck.close();
        crimeSet.load(argv[firstInputFile]);
    } else {
        cerr << "ERROR: Cannot open file: " << argv[firstInputFile] << endl;
        return 1;
    }

    for (int j = firstInputFile + 1; j < argc; j++) {
        fcheck.open(argv[j]);
        if (fcheck) {
            fcheck.close();
            currentCrimeSet.load(argv[j]);
            crimeSet += currentCrimeSet;
        } else {
            cerr << "ERROR: Cannot open file: " << argv[j] << endl;
            return 1;
        }
    }

    CrimeCounter counter(rows, cols, bottonLeftCoor, topRightCoor);
    counter.calculateFrequencies(crimeSet);
    ...
    string comment = "Running parameters: " + parametros + "\n";
    if (comment_extra != "") {
        comment += comment_extra + "\n";
    }

    comment = FormatAsComment(comment);

    ColorPalette palette(tonesD, tonesB, tonesG, tonesY);
    counter.saveAsPPMTextImage(ofilename, palette, comment);
